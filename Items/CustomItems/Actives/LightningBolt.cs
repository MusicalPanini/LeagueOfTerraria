        ...
        public override void DoActive(Player player, LeagueItem modItem)
        {
            ...
            if (cooldownCount <= 0)
            {
                ...
                if (npc != -1)
                {
                    ...
                    Projectile.NewProjectile(player.GetProjectileSource_Item(modItem.Item), Main.npc[npc].Center, Vector2.Zero, ProjectileType<Item_LightningBolt>(), damage, 0, player.whoAmI, npc);
                    SetCooldown(player);
                }
            }
        }
