...
namespace TerraLeague.Items.SummonerSpells
{
    public class ReviveRune : SummonerSpell
    {
        ...
        static public void Efx(Player player)
        {
            ...
            for (int j = 0; j < 18; j++)
            {
                Dust dust = Dust.NewDustDirect(new Vector2(Main.rand.Next((int)player.position.X - 8, (int)player.position.X + 8), player.position.Y + 16), player.width, player.height, DustID.AncientLight, 0, -Main.rand.Next(6, 18), 0, new Color(68, 253, 180, 0), Main.rand.Next(2, 3));
                dust.noGravity = true;
            }
        }
