...
namespace TerraLeague.Items.Weapons.Abilities
{
    public class ReapingSlash : Ability
    {
        ...
        {
            return "TerraLeague/AbilityImages/ReapingSlash";
        }
        ...
        public override void DoEffect(Player player, AbilityType type)
        {
            if (CheckIfNotOnCooldown(player, type) && player.CheckMana(GetBaseManaCost(), true))
            {
                ...
                Projectile.NewProjectile(player.GetProjectileSource_Item(abilityItem.Item), position, velocity, projType, damage, knockback, player.whoAmI, player.velocity.X > 0 ? 1 : -1);
