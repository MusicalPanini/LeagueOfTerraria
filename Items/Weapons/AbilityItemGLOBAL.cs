        ...
        public override void ModifyTooltips(Item item, List<TooltipLine> tooltips)
        {
            if (IsAbilityItem)
            {
                ...
                if (tt != null)
                {
                    ...
                    {
                        text += "\n[c/" + LeagueTooltip.PulseText(LeagueTooltip.ConvertHexToColor(AbilityNameColor)).Hex3() + ":Ability " + TerraLeague.ConvertKeyString(TerraLeague.QAbility) + ":] [c/" + LeagueTooltip.PulseText(LeagueTooltip.ConvertHexToColor(AbilityButtonColor)).Hex3() + ":" + GetAbility(AbilityType.Q).GetAbilityName() + "]" +
                            "\n" + GetAbility(AbilityType.Q).GetTooltip();
                    }
                    if (GetIfAbilityExists(AbilityType.W))
                    {
                        text += "\n[c/" + LeagueTooltip.PulseText(LeagueTooltip.ConvertHexToColor(AbilityNameColor)).Hex3() + ":Ability " + TerraLeague.ConvertKeyString(TerraLeague.WAbility) + ":] [c/" + LeagueTooltip.PulseText(LeagueTooltip.ConvertHexToColor(AbilityButtonColor)).Hex3() + ":" + GetAbility(AbilityType.W).GetAbilityName() + "]" +
                            "\n" + GetAbility(AbilityType.W).GetTooltip();
                    }
                    if (GetIfAbilityExists(AbilityType.E))
                    {
                        text += "\n[c/" + LeagueTooltip.PulseText(LeagueTooltip.ConvertHexToColor(AbilityNameColor)).Hex3() + ":Ability " + TerraLeague.ConvertKeyString(TerraLeague.EAbility) + ":] [c/" + LeagueTooltip.PulseText(LeagueTooltip.ConvertHexToColor(AbilityButtonColor)).Hex3() + ":" + GetAbility(AbilityType.E).GetAbilityName() + "]" +
                            "\n" + GetAbility(AbilityType.E).GetTooltip();
                    }
                    if (GetIfAbilityExists(AbilityType.R))
                    {
                        text += "\n[c/" + LeagueTooltip.PulseText(LeagueTooltip.ConvertHexToColor(AbilityNameColor)).Hex3() + ":Ability " + TerraLeague.ConvertKeyString(TerraLeague.RAbility) + ":] [c/" + LeagueTooltip.PulseText(LeagueTooltip.ConvertHexToColor(AbilityButtonColor)).Hex3() + ":" + GetAbility(AbilityType.R).GetAbilityName() + "]" +
