...
namespace TerraLeague.Projectiles.Whip
{
	public abstract class WhipProjectile : ModProjectile
	{
		abstract internal Rectangle HandleFrame { get; }
		abstract internal Rectangle Segment1Frame { get; }
		abstract internal Rectangle Segment2Frame { get; }
		abstract internal Rectangle Segment3Frame { get; }
		abstract internal Rectangle TipFrame { get; }
		...
		public override bool PreDraw(ref Color lightColor)
		{
			...
			for (int i = 0; i < list.Count - 1; i++)
			{
				...
				if (i == list.Count - 2)
				{
					...
					GetWhipSettings(out float timeToFlyOut, out int _, out float _);
		...
		public virtual void GetWhipSettings(out float timeToFlyOut, out int segments, out float rangeMultiplier)
		{
			timeToFlyOut = (float)(Main.player[Projectile.owner].itemAnimationMax * Projectile.MaxUpdates);
			segments = 20;
			rangeMultiplier = 1f;
		}
	}
