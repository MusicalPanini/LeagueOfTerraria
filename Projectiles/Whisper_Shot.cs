...
namespace TerraLeague.Projectiles
{
    public class Whisper_Shot : ModProjectile
    {
        ...
        public override void Kill(int timeLeft)
        {
            for (int i = 0; i < 10; i++)
            {
                Dust.NewDust(new Vector2(projectile.position.X, projectile.position.Y), projectile.width, projectile.height, DustID.GoldCritter, projectile.velocity.X / 5, projectile.velocity.Y / 5, 100, default(Color), 0.7f);
            }
        }
